...
void loadMatrixFile( char *filename, int *z, int matWidth, int matHeight ){
	...
	fscanf( ptr1, " %d", &x  );
	while( !feof( ptr1 ) && y < ( matWidth * matHeight ) + offset + 1 ){
		if ( y > offset ){
...
//		matB = ( int * ) malloc( sizeof ( int ) );
		...
		//load matrices from files
			...
			int *matA = ( int * )malloc( sizeof( ( matWidthA * matHeightA ) + offset ) );
			loadMatrixFile( filename1, matA, matWidthA, matHeightA );
			printMatrix( matA, matHeightA, matWidthA );
			
			int *matB = ( int * )malloc( sizeof( ( matWidthB * matHeightB ) + offset ) );
