...
__global__ void MatrixAddKernel ( int  *Md, int *Nd, int *Pd, int Width, int TILE_WIDTH ){
...
void MatrixMul( char *filename0, char *filename1, char *filename2, int Width, int TILE_WIDTH, char *Cnext ){
...
int main ( int argc, char *argv[ ] ) {
	if ( argc < 7 ){
		printf( "\n Format: %s configurationVector spikingVector spikingTransitionMatrix squareMatrixWidth TILE_WIDTH outputfile\n", argv[ 0 ] );
		exit( 1 );
	}
	char *confVec = argv[ 1 ];
	char *spikVec = argv[ 2 ];
	...
	//				printf( "\nYou have entered files %s, %s, and %s and square matrix width %d \n", spikVec, confVec, spikTransMat, width );
					...
					if ( ptr1 == 0 && ptr2 == 0 && ptr3 == 0 ) {
						...
						MatrixMul( confVec, spikVec, spikTransMat, width, TILE_WIDTH, Cnext );
					}
