...
\setcounter{page}{1}
\authorrunning{Francis Cabarle et al.}
\titlerunning{Simulating Spiking Neural P systems without delays using GPGPUs}
...
\title*{Simulating Spiking Neural P systems without delays using GPGPUs } %\titlenote{Partly supported by the \textit{UP Engineering Research and Development for Technology (ERDT)} program.}\\}
...
This paper presents a parallel simulator for a type of P system known as spiking neural P system (SNP system) using \sredmark{general purpose graphics processing units (GPGPUs)}{} \todo{GPGPU is not a device, it's the technique for General Purpose computation on GPUs}. \sredmark{GP}{GPUs}, unlike the more conventional and general purpose, multi-core CPUs, are \sredmark{used}{well-suited} for parallelizable problems due to their architectural optimization for parallel computations.
...
The matrix representation of SNP systems provides a crucial step into their simulation on parallel devices such as \sredmark{GP}{GPUs}. Simulating the highly parallel nature of SNP systems necessitates the use of hardware intended for parallel computations. The \sredmark{simulator}{simulation} algorithms, design considerations, and implementation are presented. Finally, simulation results, observations, and analyses using an SNP system that generates all numbers in $\mathbb N$ - \{$1$\} are discussed.\todo{We should consider the measure of simulation time, with a CPU-GPU comparative, so we can validate the decision of implementing the simulation algorithm on the GPU.}
...
SN P systems differ from other types of P systems precisely because they are mono-membranar and \sredmark{only use one type of object in their computations}{the working alphabet contains only one object type}. These characteristics, among others, are meant to capture the workings of a special type of cell known as the $neuron$. Neurons, such as those in the human brain, communicate or 'compute' by sending indistinct electro-chemical  signals more commonly known as action potential or \textit{spikes} \cite{snp}. Information is then communicated and encoded not by the spikes themselves, since the spikes are unrecognizable from one another, but by means of time duration, as well as the number of spikes sent/received from one neuron to another, oftentimes under a certain time interval \cite{snp}. The time duration between two spikes, or several successive spikes, transmit the information.  

It has been shown that SN P systems, given their nature, are representable by matrices \cite{snpbrain}\cite{snpmat}. This representation allows design and implementation of an SN P system simulator using parallel computing machines such as \sredmark{GP}{GPUs}. 

%\pagebreak

\subsection{Simulating SNP systems in \sredmark{GP}{GPUs}}
A matrix representation of SN P systems seem somewhat intuitive due to their graph-like nature and properties (as will be further shown in the succeeding sections such as in subsection \ref{computesnp}). \sredmark{Matrix operations, algorithms and such in parallel computing literature (e.g. \cite{matrixgpu1} and \cite{matrixgpu2} to name a few recent ones done on GPGPUs) have been studied and are available in vast quantities for years now}{On the other side, linear algebra operations have been efficiently implemented on parallel platforms in the past years. For instance, there is a large number of algorithms implementing matrix-matrix and vector-matrix operations on the GPU. These algorithms offer huge performance since dense linear algebra readily maps to the data-parallel architecture of the GPUs \cite{matrixgpu1,matrixgpu2}}\todo{I suggest to better introduce the efficient behaviour of GPUs with matrix operations, since it's the key of the paper.}. It would thus seem then that a matrix represented SN P system simulator implementation on highly parallel computing devices such as \sredmark{GP}{GPUs} be a natural confluence of the previous points made. The matrix representation of SN P systems bridges the gap between the theoretical yet still computationally powerful SN P systems and the applicative and more tangible \sredmark{GP}{GPUs}, via an SN P system simulator. \sredmark{The design of the simulator, including the algorithms deviced, architectural considerations, are then implemented using a particular type of GPGPU, namely NVIDIA CUDA (compute unified device architecture). NVIDIA CUDA extends the widely known ANSI C programming language and allows programmers to perform parallel computations, via GPGPUs manufactured by NVIDIA}{} \todo{I suggest to introduce CUDA in a new paragraph in a more extensive way, as follows}.

\redmark{The design and implementation of the simulator, including the algorithms deviced, architectural considerations, are then implemented using CUDA. The Compute Unified Device Architecture (CUDA) programming model, launched by Nvidia in mid-2007, is composed as both, hardware and software architecture for issuing and managing computations on their most recent GPU families (G80 family onward), making it operate as a highly parallel computing device. CUDA programming model extends the widely known ANSI C programming language, allowing programmers to easily design the code to be executed on the GPU, avoiding the use of low-level graphical primitives.}
...
The objective of this work is to start the foundations for the creation of P system simulators \todo{we should add a paragraph/subsection introducing previous work of simulators based on GPU, FPGA, clusters... if you agree, I can write it from my previous papers.}, in this particular case an SN P system, using highly parallel devices such as \sredmark{GP}{GPUs}. Correctness, in terms of implementation algorithms and hardware design considerations, of the output and fidelity to the computing model is a part of this objective.
...
belong to a single device i.e. a single \sredmark{GP}{GPU}. Each
device/\sredmark{GP}{GPU} has multiple cores, each capable of running
its own \sredmark{grids}{threads}\todo{I don't know in the new CUDA 4.0, but each grid is executed at one time in a GPU, where each core executes the instruction of single threads.}. The program run in the CUDA model scales
...
the host and the device parts. \redmark{The host controls the execution flow while the device
is a highly-parallel co-processor.}\todo{to define which is the role of the host and the device.} Device
pertains to the \sredmark{GP}{GPU}/s of the system, while the host
...
A kernel function call uses the \sredmark{double < and >}{triple $<$ and $>$} operator, in
this case the kernel function 

\sredmark{$add< < N, 1 > >$}{$add< < < N, 1 > > >$}$( dev\_a, dev\_b,
dev\_c )$.
...
\redmark{Since t}he kernel function \sredmark{i.e. code that}{} is executed in parallel in
the device, \redmark{it} needs to have its \sredmark{results}{inputs}\todo{first, we have to copy to the device the input data of the kernel} initially moved from the
CPU/host to the device, and then back from the device to the
host after computation\redmark{ for the results}. This movement of data back and
...
In order to represent $C_k$, $S_k$, and $M_{\Pi}$, text files are created \sredmark{in
order}{}\todo{so much ``in order'' in this sentence} to house each input, whereby each element of the
...
\subsection{Matrix computations and CPU-\sredmark{GP}{GPU} interactions}
...
element is in one thread of the device\sredmark{/GPGPU}{}), and then the
...
SNP systems \sredmark{currently}{} without time delays, is capable of
...
\subsection{\sredmark{Algorithm simulator implementation}{Simulation algorithm implementation}\todo{I prefer the term ``simulation algorithm'' instead of ``algorithm simulator''}}\label{snp-sim-algo}
	
The general algorithm is shown in Algorithm \ref{sim-algo}. \redmark{Each line in Algorithm \ref{sim-algo} mentions
which part/s the simulator code runs in, either in the device \textbf{(DEVICE)} or in the host \textbf{(HOST)} part.} \todo[color=blue]{text moved from below}
	
%\begin{algorithm}
%\caption{Overview of the algorithm for the SNP system simulator}
\todo[color=red]{I'm sorry, I had to comment the begin\{algorithm\} since latex couldn't compile it (Float(s) lost error)}.
\label{sim-algo}
\begin{algorithmic}
\item [\textbf{Require}]: creation of files $confVec$, \redmark{$M$}\todo[color=yellow]{$M$ is not defined here, it's defined later}, and $r$\\

\item [I.] \textbf{(HOST)} Load inputs: configuration vector
...
once since they are unchanging \sredmark{for}{in} an SN P system \redmark{computation}\todo[color=green]{these matrices remain unchanged during the computation of the SNP system}\\


\item [II.] \textbf{(HOST)} Determine if a rule/element in $r$ 
...
\item [III.] \textbf{(DEVICE)} From part II., run the kernel 
...
\item [IV.] \textbf{(HOST+DEVICE)} Repeat steps I to IV (except instead of loading $C_0$ as $confVec$, use the generated $C_k$s in III) until
...
\Sigma =  |r| 
\end{equation}
\todo{This number $|r|$ is also defined as number $m$ in the SNP system $\Pi=(O,\sigma_1,\ldots, \sigma_m, syn, in, out)$, should we consider to use this number instead or additionally?}
\redmark{
\begin{equation}\label{psi}
\Psi = |{\sigma_{V_1}}| |{\sigma_{V_2}}| ...|{\sigma_{V_n}}|, n \in \mathbb{N}, n \leq \infty
\end{equation}
}\todo{however, $n$ is actually $\Sigma$ (the number of neurons), what about replacing $n \leq \infty$ by $n=\Sigma$?}

where

$|{\sigma_{V_n}}|$

means the total \sredmark{count of the}{} number of rules in the $nth$ neuron which satisfy the regular expresion $E$ in (b-3). $\Sigma$ gives the total number of neurons, while $\Psi$ gives the expected number of valid and possible $S_k$s which should be produced in a given configuration.
...
and possible spiking vectors given input files $M$ \redmark{(the $list$ version of (\ref{row-maj}))}\todo{this definition can be moved above, next to algorithm 1?} , $confVec$, and $r$.
...
Using a highly parallel computing device such as a \sredmark{GP}{GPU}, \redmark{and the}
\sredmark{particularly}{} NVIDIA CUDA \redmark{programming model}, an SN P system simulator was
...
of values (vector/matrix elements) \redmark{has} provided the necessary
...
manipulation of the NVIDIA CUDA enabled \sredmark{GP}{GPU} which
took care of repetitive and highly parallel computations
(\redmark{vector-matrix} addition and multiplication essentially).

Future versions of the SNP system simulator will focus on
several improvements. These improvements include the use
of an \redmark{optimized} algorithm for matrix computations \redmark{on the GPU} without requiring
...
\sredmark{GP}{GPUs} \sredmark{running}{supporting} NVIDIA CUDA.
...
The authors from UP Diliman are supported by the \textit{ERDT program}. They also wish to acknowledge the \textit{Algorithms and Complexity laboratory} of the \textit{UP Diliman Department of Computer Science} for the use of Apple iMacs with NVIDIA CUDA enabled GPUs, which provided the proper environment for the simulations, their development, design and tests. \redmark{The last author acknowledge the support of the project TIN2009--13192 of the Ministerio de Educaci\'on y Ciencia of Spain, and the support of the ``Proyecto de Excelencia con Investigador de Reconocida Val\'ia'' of the Junta de Andaluc\'{\i}a under grant P08-TIC04200, both co-financed by FEDER funds.}\todo{Please, I will thank you a lot if we could add the acknowledge of Sevilla's projects, which is very important for my predoctoral grant.}
...
\bibitem{ppage} P systems resource website. (\sredmark{2010}{2011?}, Jan) [Online]. Available: {\tt www.ppage.psystems.eu}.
