...
__global__ void MatrixAddKernel ( int  *Md, int *Nd, int *Pd ){
    ...
    for ( int k = 0; k < %(MATRIX_SIZE)s; ++k ){
            ...
            Pd[ row * %(MATRIX_SIZE)s + k  ] = Md[ row * %(MATRIX_SIZE)s + k ] + Nd[ row * %(MATRIX_SIZE)s + k ];
    }
} 
 """
...
def genSpikRuleList( confVec, neurNum ) :
...
#		tmpList[ idx ][ 0 ] = spike
	for idx2, neuron in enumerate( ruleregexp ) :
		...
		for idx3, rule in enumerate( neuron ) :
			...
			regexp = ruleregexp[ idx2 ][ idx3 ]
			regexp = regexp.split( )
			if chkRegExp( regexp[ 0 ], int( spike ) ) :
				sameCnt += 1
				tmpList[ idx2 ][ y ] = sameCnt
			else :
				tmpList[ idx2 ][ y ] = 0
...
def addTotalCk( allGenCk, Ck_1_str, allGenCkFile ) :
