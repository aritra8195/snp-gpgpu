...
sections) (\ref{confvec}) and (\ref{rule-list}) will be utilized. For part $II$ Listing \ref{sim-algo} we have the following sub-algorithm for generating all valid
and possible spiking vectors given input files $M$ (the $list$ version of (\ref{row-maj})) , $configVec$, and $r$:

\lstset{numbers=left, numberstyle=\tiny, stepnumber=0, numbersep=5pt, label=cuda_gencode}
% This concludes the preambleII-1 Create a list tmp, a copy of r, marking each element of
tmp in increasing order of N, as long as the element/s
satisfy the rule's regular expression E of a rule (given by list
...
then
tmp = [ [ 2, 2 ], [ 1 ], [ 1, 2 ] ]
and that given (a) we thus have the following passes as we
traverse tmp, with underlined elements being the elements
that are currently being checked at that stage/pass:
1st pass:
tmp = [ [ 1, 2 ], [ 1 ], [ 1, 2 ] ]
Remark/s: previously, tmp [ 0 ][ 0 ] was equal to 2,
...
2nd pass:
tmp = [ [ 1, 2 ], [ 1 ], [ 1, 2 ] ]
Remark/s: previously tmp[ 0 ][ 1 ] = 2, which has
...
5th pass:
tmp = [ [ 1, 2 ], [ 1 ], [ 1, 0 ] ]
Remark/s: element tmp[ 2 ][ 1 ], or the 2nd
element/rule of neuron 3 doesn't satisfy E.
Final result:
tmp = [ [ 1, 2 ], [ 1 ], [ 1, 0 ] ]
At this point we can observe the following, based on the
earlier definitions:
Σ = 3 ( 3 neurons in total, one per element/value of
confVec )
Ψ = |σV1| |σV2| |σV3| = 2 * 1 * 1 = 2
Ψ tells us the number of valid strings of 1s and 0s i.e.
spiking vectors, which needs to be produced later, for a
given configuration, in this case we have (a). There are only
2 valid spiking vectors from configVec = [ 2, 1, 1 ] and the
rules given in (b) encoded in r. These spiking vectors are
01 1 10
10 1 10


\begin{lstlisting}[label=sim-algo2,caption=Detailed algorithm for part $II$ in Listing \ref{sim-algo}]

	
II-1 Create a list tmp, a copy of r, marking
each element of tmp in increasing order of N,
